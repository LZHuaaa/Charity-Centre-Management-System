...
public class DonorDAO {
            ...
            while ((line = reader.readLine()) != null) {
                if (line.equals("END_OF_DONOR")) {
                    ...
                    if (parts.length == 4 && donor == null) {
                        ...
                        item = new DonatedItem(parts[0], Integer.parseInt(parts[1]));
                        items.add(item);
                    }
    ...
    //load donation to donation.txt for donee
    public static void processAndSaveDonations(String inputFilePath, String outputFilePath) {
        try (Scanner scanner = new Scanner(new File(inputFilePath)); FileWriter writer = new FileWriter(outputFilePath)) {
            ...
            while (scanner.hasNextLine()) {
                ...
                if (line.startsWith("D")) {
                    String[] donationDetails = line.split(",");
                    String donationID = donationDetails[0];
                    String donationName = donationDetails[1];
                    String donationAmount = donationDetails[2];

                    donationData.append(donationID)
                            .append(",")
                            .append(donationName)
                            .append(",")
                            .append(donationAmount)
                            .append("\n");
                } else if (!line.equals("END_OF_DONATION") && !line.equals("END_OF_DONOR")) {
                    String[] itemDetails = line.split(",");
                    String itemName = itemDetails[0];
                    String itemQuantity = itemDetails[1];
            ...
            writer.write(donationData.toString());
